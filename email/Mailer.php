...
class Mailer extends Object {
	...
	/**
	 * Does all the preparation stuff for sending a HTML email (generating headers, encoding, handling attachments, etc)
	 * 	without actually sending an email. Used by htmlEmail()
	 * @param unknown $to
	 * @param unknown $from
	 ...
	 * 
	 */
	static function prepareHTML($to,$from,$subject,$htmlContent,$attachedFiles = False,$customheaders = False,$plainContent = false,$inlineImages = false) {
...
function htmlEmail($to, $from, $subject, $htmlContent, $attachedFiles = false, $customheaders = false, $plainContent = false, $inlineImages = false) {
	...
	$data = Mailer::prepareHTML($to, $from, $subject, $htmlContent,$attachedFiles,$customheaders,$plainContent,$inlineImages);
	
	// Try it without the -f option if it fails
	if(!($result = @mail($data['to'], $data['subject'], $data['fullBody'], $data['headers'], "-f" . $data['bounceAddress']))) {
		$result = mail($data['to'], $data['subject'], $data['fullBody'], $data['headers']);
	}
	 ...
	 */
	//$result = QueuedEmail::send($data['to'], $data['subject'], $data['fullBody'],$data['headers'],$data['bounceAddress']); 
